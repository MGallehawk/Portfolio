...
$IntuneExportPath = 'C:\Users\e103719\G.James Australia Pty Ltd\Team ICT Service Desk - General\ICT Service Desk technical Library\Service Desk Technical Documents\6 - Project Work\Asset Management\platform\itteration (2)\test\make array\intune-export.csv'

#location of ouputed data - asset list transformed for snow
$SnowConversionOutpath = 'C:\Users\e103719\G.James Australia Pty Ltd\Team ICT Service Desk - General\ICT Service Desk technical Library\Service Desk Technical Documents\6 - Project Work\Asset Management\platform\itteration (2)\test\make array\output.csv'
...
#Assigning Variables to Object
  ...
  $DeviceObject | Add-Member -MemberType NoteProperty -Name 'Model Catagory' -Value $ModelCat
...
$DeviceObject | Select-Object * | Out-File -FilePath $SnowConversionOutpath -Append
#>

#filter corperate only devices and devices named Deskop that have not enrolled properly
if ($DeviceObject.'Owned By' -eq 'Ross Lenox' -And $DeviceObject.'Asset tag' -NotLike 'DESKTOP-*')
  {
...
{
    $DeviceObject |  Select-Object * | Export-Csv -Path $SnowConversionOutpath -Append -NoTypeInformation
