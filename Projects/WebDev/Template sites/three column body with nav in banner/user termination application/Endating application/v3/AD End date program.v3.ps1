...
notes: 
 v3 has been arrested as currently domain controller is not set up to access exchange online. this will require cab.. i have little faith that this change request will be granted in the near future.
 have created a seperate application that has this functionality to be run outside the domain controller
...
$ValButton.Add_Click(
    {
        ...
        #debug 
        #############
        $DebugnMessageLabel1.Text = "First name of AD user: " + $fn;
        $DebugnMessageLabel2.Text = "Last name of AD user: " + $LN;
        $DebugnMessageLabel3.Text = "First name of Provided: " + $FNameTextBox.Text;
        $DebugnMessageLabel4.Text = "Last name provided: " + $LNameTextBox.Text;
        $DebugnMessageLabel5.Text = "Ritm number provided " + $RITMTextBox.Text;
        $DebugnMessageLabel6.Text = "System Date: " + $Date;
        $DebugnMessageLabel7.Text = "Description: " + $Description;
        #############
       ...
       if ( $validationLabel.Text -eq $Vtrue )
            {
                ...
                $endDateOU = "OU=End-dated Users,OU=Corporate,DC=MatsLabDomain,DC=com";# test enviromet
                $RITM2 = $RITMTextBox.Text;
                $Description2= ("Disabled on: " + $Date + " for " + $RITM2);
                #$dlGroups = get-adgroup -filter {name -like "DL -*"};
                #$365Licenses = get-adgroup -filter {name -like "*365*"};
                #$project = get-adgroup -filter {name -like "*project*"};
...
$EmailButton.Add_Click(
    {
    $deadEmail = Get-adUser -Identity $EnumberTextBox.Text.Trim();
    $ManagerEmail = Get-adUser -Identity $emailDelegateEnumber.Text.Trim();
    $deadcheck = $false;
    $managercheck = $false;
