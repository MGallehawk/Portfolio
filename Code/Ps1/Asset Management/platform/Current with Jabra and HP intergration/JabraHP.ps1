...
#feilds indicate the defult location of the Intun, Sophos, HP and Jabra , output folders
...
#Used for error handeling messages
...
try {
  ...
  $IntuneImportError = [System.Windows.MessageBox]::Show('No Intune Import file detected, please input the required csv file','Intune Import error','OK','Error') 
...
$SophosInport = Import-Csv -Path $sophosCsvFiles.FullName  
  ...
  #error message for intune no csv
  $SophosImportError = [System.Windows.MessageBox]::Show('No Sophos Import file detected, please input the required csv file','Sophos Import error','OK','Error') 
  switch  ($msgBoxInput) { 'OK' { Exit;}}
  $SophosImportError
  #Disconnect-AzureAD
  Exit
...
try {
  ...
  #error message for Jabra no csv
...
#device name for phones
$phoneName = $AssignedTo + ' Mobile Phone'
...
#logic block for differentiating between phone and computer and setting naming conention for phones
  If ($DevOS -eq 'Windows' -Or $DevOS -eq 'MacOS') { $ModelCatMU = 'Computer'} 
  else {$ModelCatMU = 'Mobile Handset'; $DevName = $phoneName}

#Logic block for diferentiation between perconal and corperate devices  
...
$JabraInport | foreach-object {
  ...
  $HPInport | foreach-object {
    ...
    #transforms manufacturers inline with snow
    ...
    #filters devices that are either samsung or HP and appends the correct ones to the object arrays for model and asset
...
#cleans up the previous run input data
Remove-Item ($IntuneInportLocation+'\'+$intuneCsvFiles) -Erroraction 'silentlycontinue'
#cleans up the previous run input data
Remove-Item ($SophosInportLocation+'\'+$sophosCsvFiles) -Erroraction 'silentlycontinue'
#cleans up the previous run input data
Remove-Item ($JabraInportLocation+'\'+$JabraCsvFiles) -Erroraction 'silentlycontinue'
#cleans up the previous run input data
Remove-Item ($HPInportLocation+'\'+$HpCsvFiles) -Erroraction 'silentlycontinue'
